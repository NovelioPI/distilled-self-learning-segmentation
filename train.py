...
class BaseModel(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        if self.entropy_threshold == "adaptive":
            self.adaptive_entropy_thresholds = 0.0
    ...
    def step(self, batch, stage="train"):
        ...
        if stage == "train":
            ...
            if self.entropy_threshold == "adaptive":
                thresholds = self.adaptive_entropy_thresholds
                mask = entropy < thresholds
                pseudo_labels[~mask] = self.ignore_index
            elif isinstance(self.entropy_threshold, (int, float)) and self.entropy_threshold < 1.0:
                thresholds = self.entropy_threshold
                mask = entropy < thresholds
                pseudo_labels[~mask] = self.ignore_index
